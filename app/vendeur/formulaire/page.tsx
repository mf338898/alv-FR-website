...
const vendeurTypeOptions: Array<{ value: VendeurType; label: string }> = [
  ...
  { value: "indivision", label: "Plusieurs vendeurs en indivision / héritiers" },
...
function SocieteSection({
  ...
  return (
    <div className="space-y-6">
      <ModernFormSection
        title="Identification de la société vendeuse"
        subtitle="Champs obligatoires"
        icon={<Building2 className="h-5 w-5" />}
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <ModernFormField label="Dénomination de la société vendeuse" required isMissing={isMissing("denomination")} fieldId={fieldId("denomination")}>
      ...
      <ModernFormSection
        title="Représentant de la société vendeuse"
        subtitle="Qui signe pour la société ?"
        icon={<Users className="h-5 w-5" />}
      >
        <div className="space-y-4">
          <ModernFormField label="Qui représente la société pour cette vente ?" required isMissing={isMissing("representantType")} fieldId={fieldId("representantType")}>
...
function PersonneMoraleSection({
  ...
  return (
    <div className="space-y-6">
      <ModernFormSection
        title="Identification de la personne morale vendeuse"
        subtitle="Champ libre pour décrire l’entité"
        icon={<FileText className="h-5 w-5" />}
      >
        <ModernFormField
          label="Merci de décrire la personne morale vendeuse (nom, forme, adresse du siège, immatriculation, références utiles…)"
          required
        >
      ...
      >
        <div className="space-y-4">
          <ModernFormField label="Qui représente cette personne morale pour cette vente ?" required>
            <Select value={data.representantType} onValueChange={(val) => update("representantType", val)}>
              <SelectTrigger>
                <SelectValue placeholder="Choisir" />
              </SelectTrigger>
...
function MineurSection({ data, onChange }: { data: MineurSeller; onChange: (data: MineurSeller) => void }) {
  ...
  return (
    <div className="space-y-6">
      <ModernFormSection title="Informations sur le mineur vendeur" icon={<User className="h-5 w-5" />}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
export default function VendeurFormPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/generer-pdf-vendeur", {
        method: "POST",
        headers: {
  ...
  const handleSaveDraft = () => {
    toast.success("Brouillon vendeur enregistré localement (aucun envoi déclenché).")
  }
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  Si vous êtes plusieurs vendeurs, chaque personne doit remplir sa propre fiche. Vous pourrez partager le lien.
                </p>
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">Combien de vendeurs sont propriétaires de ce bien ?</label>
              <Input
                type="number"
                min={1}
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-8">
        <Card className="p-4 sm:p-6 shadow-lg border-0 bg-gradient-to-br from-purple-50/60 to-pink-50/40">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              <h2 className="text-lg font-semibold text-purple-900">Nombre de vendeurs et partage du formulaire</h2>
        ...
        <ModernFormSection
          ...
          subtitle="Type de vendeur (V0), nombre de vendeurs (G0) et identification rapide (G1)"
          icon={<Tag className="h-5 w-5 text-purple-600" />}
          isActive
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ModernFormField label="Qui est le vendeur ?" required>
              <Select value={state.type} onValueChange={(val: VendeurType) => handleTypeChange(val)}>
                <SelectTrigger className="h-12">
                  <SelectValue placeholder="Choisir un type de vendeur" />
                </SelectTrigger>
                <SelectContent>
            ...
            <ModernFormField label="Combien de vendeurs sont propriétaires de ce bien ?" required>
              <Input
                type="number"
                min={1}
              ...
              />
              {isCouple && <p className="text-xs text-slate-500 mt-1">Fixé à 2 vendeurs pour la branche couple.</p>}
              {state.type === "personne_seule" && (
                <p className="text-xs text-slate-500 mt-1">Fixé à 1 vendeur pour une personne seule.</p>
              )}
              {state.type === "indivision" && (
                <p className="text-xs text-slate-600 mt-2">
                  Indiquez le nombre total de vendeurs (indivision, héritiers...). Chaque personne remplira sa fiche.
                </p>
              )}
            </ModernFormField>
          </div>
                    ...
                    <div key={`vendeur-${index}`} className="p-4 border border-slate-200 rounded-lg bg-white shadow-sm">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm font-semibold text-slate-800">Vendeur {index + 1}</p>
                        </div>
                      ...
                      {isIndivision && (
                        <div className="mt-3 pt-3 border-t border-slate-200">
                          <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                            <input
                              type="checkbox"
                              className="h-4 w-4 rounded border-slate-300 text-purple-600 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed"
                            ...
                            <span className={isVendeur1 ? "text-slate-500" : ""}>
                              {isVendeur1 ? "Remplir la fiche de ce vendeur (obligatoire)" : "Remplir la fiche de ce vendeur"}
                            </span>
                          </label>
                        </div>
                      )}
        ...
        {state.type === "personne_seule" ? (
          <PersonSellerCard
            title="Fiche vendeur – personne seule"
            data={state.personne}
        ...
        {isCouple && (
          <div className="space-y-6">
            ...
            <div className="rounded-lg border border-purple-200 bg-purple-50 p-4 text-sm text-purple-800 flex items-start gap-3">
              <BadgeInfo className="h-5 w-5 mt-0.5" />
              <div>
                <p>Pour gagner du temps, la situation matrimoniale du vendeur 2 peut être synchronisée automatiquement.</p>
                <label className="flex items-center gap-2 mt-2 text-purple-900">
                  <input
                    type="checkbox"
                    className="h-4 w-4"
                  ...
                  />
                  Copier la situation matrimoniale du vendeur 1 vers le vendeur 2
                </label>
              </div>
            </div>

            <PersonSellerCard
              title="Fiche couple – Vendeur 1"
        ...
        {isIndivision && (
          <div className="space-y-4">
            <div className="flex items-start gap-2 bg-purple-50 border border-purple-100 rounded-lg px-4 py-3">
              ...
              <div className="space-y-1">
                <p className="text-sm font-semibold text-slate-900">Chaque vendeur doit remplir sa fiche</p>
                <p className="text-sm text-slate-700">
                  Le vendeur qui remplit actuellement est considéré comme « Vendeur 1 ». Il peut compléter uniquement sa fiche ou,
                  s&apos;il le souhaite, remplir également celles des autres vendeurs.
                </p>
              </div>
            </div>
            <div className="space-y-6">
              {resizePersonList(state.indivision, state.nombreVendeurs)
                ...
                .map(({ personne, index }) => (
                  <PersonSellerCard
                    ...
                    title={`Fiche vendeur ${index + 1} ${index === 0 ? "(vous)" : ""}`.trim()}
                    data={personne}
            ...
            {state.vendeursARemplir.filter((v, idx) => idx > 0 && !v).length > 0 && (
              <div className="rounded-lg bg-slate-50 border border-slate-200 px-4 py-3 text-sm text-slate-700">
                Pensez à partager le lien de ce formulaire aux autres vendeurs pour qu&apos;ils remplissent leur fiche de renseignements
                de leur côté.
              </div>
            )}
          </div>
