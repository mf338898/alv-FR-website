...
async function drawSingleLeftAlignedColumnWithBreaks(ctx: DocContext, p?: any, personTitle: string = "Vendeur") {
  ...
  const situationRows: Row[] = [{ label: "Situation matrimoniale", value: pdfSafe(showOrDash(p?.situationMatrimoniale)) }]

  const d = p?.situationDetails || {}
  const situationPhrase = (() => {
    switch (p?.situationMatrimoniale) {
      case "marie":
        return nonEmpty(d.conjointNomPrenom) ? `Se déclarant marié à ${d.conjointNomPrenom},` : "Se déclarant marié,"
      case "pacs":
        return nonEmpty(d.partenaireNomPrenom)
          ? `Célibataire soumis à un Pacte civil de solidarité conclu avec ${d.partenaireNomPrenom}, ainsi déclaré,`
          : "Célibataire soumis à un Pacte civil de solidarité, ainsi déclaré,"
      case "celibataire":
        return "Célibataire,"
      case "veuf":
        return nonEmpty(d.epouxDecedeNomPrenom) ? `Veuf de ${d.epouxDecedeNomPrenom}, non remarié,` : "Veuf non remarié,"
      case "divorce":
        return nonEmpty(d.exConjointNomPrenom) ? `Divorcé de ${d.exConjointNomPrenom}, non remarié,` : "Divorcé, non remarié,"
      case "autre":
        return nonEmpty(d.autreDescription) ? `${d.autreDescription},` : ""
      default:
        return ""
    }
  })()

  if (nonEmpty(situationPhrase)) {
    situationRows.push({ label: "Mention", value: pdfSafe(situationPhrase) })
  }
  ...
  const clauseLabel =
    rep.clause === "oui"
      ? "Oui"
      : rep.clause === "habilitation"
        ? "Oui, habilitation familiale"
        : "Non"
  const representationRows: Row[] = [{ label: "Clause de représentation", value: pdfSafe(clauseLabel) }]

  if (rep.clause === "non") {
    representationRows.push({ label: "Mention", value: pdfSafe("Aucune clause de représentation n'est insérée.") })
  }
  ...
  if (rep.clause === "oui") {
    representationRows.push({
      label: "Mention",
      value: pdfSafe(
        `Représenté par ${showOrDash(rep.nom)} ${showOrDash(rep.prenom)} demeurant ${showOrDash(
          rep.adresse
        )} en vertu du pouvoir joint. Téléphone : ${showOrDash(rep.telephone)} – Email : ${showOrDash(rep.email)}`
      ),
    })
  }

  if (rep.clause === "habilitation") {
    representationRows.push({
      label: "Mention",
      value: pdfSafe(
        `Représenté par ${showOrDash(rep.nom)} ${showOrDash(
          rep.prenom
        )}, au titre d'une mesure d'habilitation familiale aux termes d'une ordonnance rendue par le juge ${showOrDash(
          rep.juge
        )} de ${showOrDash(rep.tribunal)} en date du ${showOrDash(rep.dateOrdonnance)}. Téléphone : ${showOrDash(
          rep.telephone
        )} – Email : ${showOrDash(rep.email)}`
      ),
    })
  }
...
async function drawSociete(ctx: DocContext, data: any) {
  ...
  await drawRowsSection(ctx, "Société propriétaire", [
...
async function drawAssociation(ctx: DocContext, data: any) {
  ...
  await drawRowsSection(ctx, "Association propriétaire", [
