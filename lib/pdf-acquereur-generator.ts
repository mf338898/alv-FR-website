...
async function drawHeader(page: any, pdf: PDFDocument, fontBold: any, fontReg: any) {
  ...
  y -= 8
  drawCenteredText(page, pdfSafe("FICHE DE RENSEIGNEMENT ACQUÉREUR"), y, fontBold, TITLE_SIZE, PRIMARY)
  y -= TITLE_SIZE

  // Small centered grey paragraph
  y -= 5
  const intro =
    "Document strictement confidentiel – destiné à un usage achat. " +
    "Ce document synthétise les informations personnelles communiquées via notre formulaire en ligne pour la rédaction du compromis de vente."
...
async function drawSociete(ctx: DocContext, data: any) {
  ...
  await drawRowsSection(ctx, "Société acquéreuse", [
...
async function drawAssociation(ctx: DocContext, data: any) {
  ...
  await drawRowsSection(ctx, "Association acquéreuse", [
...
async function drawMineur(ctx: DocContext, data: any) {
  ...
  await drawRowsSection(ctx, "Mineur acquéreur", [
...
// Filename
export function buildAcquereurPdfFilename(data: any): string {
  const names: string[] = []
  ...
  return `FR-acquereur-${safe || "Acquereur"}.pdf`
}

// Main generator
export async function generateAcquereurPdf(data: any): Promise<Buffer> {
  ...
  if (data?.type === "personne_seule") {
    ...
    const acquereur1 = data?.couple?.vendeur1
    const acquereur2 = data?.couple?.vendeur2
